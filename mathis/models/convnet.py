...
from keras.layers import Conv2D  # Changé de Convolution2D à Conv2D
...
def ConvNet(name, input_shape, filter_shapes, fc_layer_sizes, \
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        for l in range(num_conv_layers):
            ...
            if l == 0:
                # Nouvelle syntaxe pour Conv2D
                model.add(Conv2D(n_channel, (height, width), 
            ...
            if batch_norm:
                ...
                model.add(BatchNormalization(name='conv_bn%d' % l, axis=-1))
            if dropout:
        ...
        for l in range(num_fc_layers):
            if l + 1 == num_fc_layers:
                bias = True
            # Nouvelle syntaxe pour Dense
            model.add(Dense(units=fc_layer_sizes[l],
                           name='dense%d' % l,
                           kernel_initializer=weight_init,
                           use_bias=bias))
            if batch_norm and l + 1 < num_fc_layers:
                model.add(BatchNormalization(name='bn%d' % l, axis=-1))
