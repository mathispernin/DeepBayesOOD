...
def logsumexp(x):
    ...
    x_ = x - x_max	# (dimY, N)
    tmp = tf.math.log(tf.clip_by_value(tf.reduce_sum(tf.exp(x_), 0), 1e-9, np.inf))
    #tmp = tf.math.log(tf.reduce_sum(tf.exp(x_), 0) + 1e-9)
    return tmp + x_max
...
def construct_optimizer(X_ph, Y_ph, enc, dec, ll, K, vae_type='A'):
    ...
    if ll in ['l1_logistic', 'l2_logistic', 'gaussian_logistic', 'laplace_logistic']:
        ...
        X_ = tf.math.log(X_) - tf.math.log(1 - X_)
        ll_ = ll.split('_')[0]
    else:
        X_ = X_ph
        ll_ = ll
    ...
    beta_ph = tf.compat.v1.placeholder(tf.float32, shape=(), name='beta')
    ...
    opt = tf.compat.v1.train.AdamOptimizer(learning_rate=lr_ph).minimize(-bound)
    ops = [opt, bound]
