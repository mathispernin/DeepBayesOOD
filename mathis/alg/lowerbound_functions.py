...
def sample_gaussian(mu, log_sig, K):
    ...
    z =  mu + tf.exp(log_sig) * tf.random.normal(tf.shape(mu)) #tf.random_normal(mu.get_shape())
    return mu, log_sig, z
...
def sample_gaussian_fix_randomness(mu, log_sig, K, seed):
    ...
    eps = np.random.randn(K, tf.shape(mu)[1])
...
def logsumexp(x):
    ...
    x_ = x - x_max	# (dimY, N)
    tmp = tf.math.log(tf.clip_by_value(tf.reduce_sum(tf.exp(x_), 0), 1e-20, np.inf))
    return tmp + x_max
...
def lowerbound_A(x, fea, y, enc_mlp, dec, ll, K=1, IS=False, 
    ...
    if IS and K > 1:	# importance sampling estimate
        ...
        bound = logsumexp(bound) - tf.math.log(float(K))
...
def lowerbound_B(x, fea, y, enc_mlp, dec, ll, K=1, IS=False, 
    ...
    if ll == 'logit_l1':
        ...
        x_rep_logit = tf.math.log(tmp) - tf.math.log(1 - tmp)
